...
% 设置根目录
rootDir = 'G:\Projects\ViT\results\distorted_with_knowledge';
% 文件名列表
fileNames = {
...
% G:\Projects\ViT\analysis\performance_knowledge 中
% === 1. 准备基准性能映射 ===
baseline_data = {
    'blur',         5,   0.8555;
    'blur',         7,   0.8384;
...
% ==== 设置保存目录 ====
saveDir = 'G:\Projects\ViT\analysis\performance_knowledge';
...
target_clustering = 'GMM_category';
knowledge_K = 100;
data = readtable('G:\Projects\ViT\results\distorted_with_knowledge\performance_none.csv');
performanceData = struct();
performanceData.none = data;
distortion_types = fieldnames(performanceData);
avg_baseline_f1 = 0.8715;  % 基准性能均值
% 创建图窗
fig = figure('Name', 'Performance vs Prior Weight (GMM\_category)', ...
...
% 保存为 PDF
exportgraphics(fig, fullfile(saveDir, 'GMM_category_PW03_Performance_vs_RetrievalK_withBaseline.pdf'), 'ContentType', 'vector');
% close(fig);
...
ylabel('Average F1 Score');
% 读取仅检索的性能
rootDir = 'G:\Projects\ViT\results\distorted_with_knowledge_only_retrieval';
fileNames = {
    'performance_blur.csv', ...
    'performance_compression.csv', ...
    'performance_illumination.csv', ...
...
%% analysis_only_retrieval_nodistortion: 分析仅检索与检索后特征融合的不同
% 结果保存目录与基准性能
saveDir = 'G:\Projects\ViT\analysis\performance_knowledge';
if ~exist(saveDir, 'dir'), mkdir(saveDir); end
avg_baseline_f1 = 0.8715;
% 读取检索后特征融合的性能
rootDir = 'G:\Projects\ViT\results\distorted_with_knowledge';
fileNames = {
...
% 路径列表
dirs = {
    'G:\Projects\ViT\results\distorted_with_knowledge', ...
    'G:\Projects\ViT\results\distorted_with_knowledge_high_weight'
};

% 加载数据
allData = load_all_tables_from_dirs(dirs);
...
% 保存图像
savePath = 'G:\Projects\ViT\analysis\performance_knowledge\GMM_category_K2000_R5_vs_PriorWeight.pdf';
...
for d = 1:length(distortion_types)
    ...
    for c = 1:length(clustering_methods)
        ...
        for p = 1:length(params)
            ...
            sel = tbl.distortion_param == param_val & strcmp(tbl.knowledge_type, clustering) & tbl.prior_weight == 0.4 & tbl.knowledge_K == 2000 & tbl.retrieval_k == 5;
...
% 绘制为虚线，同一个知识库规模采用同一个颜色绘制。此外，绘制一条水平虚线表示 baseline 性能，性能为 0.8715。以 PDF
% 格式保存绘制结果
% 读取CSV文件数据
data = readtable('G:\Projects\ViT\results\distorted_with_knowledge\performance_none.csv');
saveDir = 'G:\Projects\ViT\analysis\performance_knowledge';
...
filtered_data = data(strcmp(data.distortion_type, 'none') & ...
                    data.distortion_param == 0 & ...
                    data.prior_weight == weight, :);
...
% 绘制基线
baseline_perf = 0.8715;
yline(baseline_perf, 'k--', 'LineWidth', 1.5, 'DisplayName', 'Baseline (0.8715)');
